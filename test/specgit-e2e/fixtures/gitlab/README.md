...
They are data only — no test imports them yet; the future `GlabProvider`
adapter is to build its contract tests against these shapes.
...
2. **Commit pass (this repository)** — fields the first pass did not cover:
   ...
   organizational free text remain; the `forge.example.com` host and structural
