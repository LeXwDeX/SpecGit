  ...
  it('exits 3 when the GitHub provider is unauthenticated', async () => {
    const t = makeCtx({
      ...
      gh: {
        ...
        getIssueWriterAuthority: async () => { throw new Error('unreachable'); },
      getRequestDeclarations: async () => ({ ok: false, code: 'gh_transport', message: 'unreachable' }),
        appendRequestDeclaration: async () => ({ ok: false, code: 'gh_transport', message: 'unreachable' }),
