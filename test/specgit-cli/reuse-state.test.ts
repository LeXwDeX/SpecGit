...
function fixture() {
  ...
  const observe = (nativeChange: object = {}) => {
    ...
    // Windows deduplicates environment names case-insensitively before spawning.
    // Remove every inherited spelling before supplying the controlled image.
    const childEnv = Object.fromEntries(Object.entries(fake.env({ ...env, GITLAB_CI: 'false',
      GITHUB_REPOSITORY: 'owner/project', GITHUB_RUN_ID: '11', GITHUB_RUN_ATTEMPT: '1',
    })).filter(([key]) => !['IMAGEOS', 'IMAGEVERSION'].includes(key.toUpperCase())));
    const output = execFileSync(process.execPath, ['--input-type=module', '-e', source], { cwd: root,
      env: { ...childEnv, ImageOS: image, ImageVersion: '20260908.1.0' },
