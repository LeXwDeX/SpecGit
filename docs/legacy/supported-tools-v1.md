> **Archived SpecGit v1 guide.** This page preserves the retired TypeScript
> integration and command workflow. Use the [current v2 agent guide](../supported-tools.md)
> and [migration guide](../migration-v2.md) for new work.
...
1. **Commands are stable.** The human story is `issue` → `finish`, with `init` and `setup` for setup, `pr`, `status`, `doctor` for repair and diagnostics, and `bind`/`unbind`/`accept` as machine aliases — see the [v1 CLI reference](../cli.md).
...
Full v1 rules: [Agent Contract](../agent-contract.md).
