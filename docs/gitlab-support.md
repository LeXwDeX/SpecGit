...
- `parseRepoRef` honors the declared endpoint: a matching origin (and `gitlab.com`) fails with `gitlab_unsupported` instead of the generic `origin_unresolvable` — the diagnostic names the actual gap, and since #78's evaluator change it surfaces under its own code through the G5 origin gate (factual, exit 1), never folded into `origin_unresolvable`. Explicit ports follow the #78 rule: a scheme-default port (`:443` https, `:22` ssh) classifies like the portless form; a non-default port classifies only when the declaration names it (`host:port`, persisted as `gitlab.port`). Since #95, **nested-group GitLab origins** (`group/subgroup/project`, any depth ≥ 2 path on the declared host or a `*gitlab*` host) also report `gitlab_unsupported` with platform-neutral fix text instead of being misdiagnosed as unresolvable URLs with GitHub-pointing advice. Undeclared non-github origins stay `origin_unresolvable` with a `platform_undecided` warning.
