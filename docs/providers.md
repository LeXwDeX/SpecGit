...
| `readFileAtCommit` | required | Read a regular file at a full commit identity. Missing objects fail closed; absent files return null. No network or ref changes. |
| `readFileHistory` | required | Complete first-parent history of a scope path at a full commit identity, oldest first including deletions. Shallow or more than 1,000 declaration revisions fail closed. |
| `readFileBeforeMerge` | required | Recover the original policy from the target parent of a merge whose second parent is the proven PR head, or provider-backed target ancestry with unchanged policy history for squash/rebase/fast-forward. This reader is independent of checkout; local completion callers separately require HEAD containment. |
