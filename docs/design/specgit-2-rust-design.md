...
| R22 | Agent 通过 CLI 进行完整机器通信；所有业务能力可发现、非交互调用、解析结果和恢复，hook 不另建一套业务协议。 |
...
### 5.4 Agent-native CLI 通信契约

用户追加确认：应用应完整支持新近强调的面向 Agent 的 CLI 路线，并要求查阅原始说明。2026-09-10 的调研中，最符合其描述的是 **Agent-native CLI**：例如 [CLI-Anything](https://github.com/HKUDS/CLI-Anything) 通过可发现命令、JSON 结果和配套 `SKILL.md` 使软件可被 Agent 直接使用；[其论文](https://arxiv.org/abs/2606.03854) 强调结构化操作、显式状态与可核对反馈。这是依据描述作出的概念对应，不把 CLI-Anything 当成用户点名指定的依赖或统一行业协议。
...
三者不是互斥方案，也不能用“加了 `--json`”就宣称 Agent 能完成任务。SpecGit 的产品组合是 **Skill 教会调用方式，CLI 提供可执行能力，hook 交付变化，Agent 决策，原生服务执行合并与自动关闭**。实际验收必须从安装、发现命令到结果恢复走完整旅程。

检索也发现了名为 [ACLI](https://github.com/lifeprompt-team/acli) 的具体项目，它在 MCP 上提供 CLI 风格命令和动态发现；这是另一种实现选择，不能混称为所有 CLI 的通用标准。此处不引入它的框架或新增服务依赖。schema 自省与 dry-run 的具体实践参考 [Google Workspace CLI](https://github.com/googleworkspace/cli/blob/main/README.md)，按 SpecGit 自身规模实现。
...
| 编号 | 必须具备的通信能力 | 具体约定 |
| --- | --- | --- |
| C01 | 命令与 schema 可发现 | 各命令 `--help`；顶层/命令 `--schema` 返回版本化输入、输出和副作用说明；这些离线查询无需认证或网络。 |
| C02 | 显式、完整输入 | 简单选项用 argv；复杂输入用 `--input-file <path>` 或 `--input-file -` 的有界 JSON stdin；与冲突参数互斥。不得等待一个未声明的 stdin。 |
| C03 | 非交互可执行 | `--json` 或非 TTY 默认机器模式；不偷偷弹提示。缺少必要选择返回完整 `confirmation_required`，Agent 利用现有授权或向用户取得缺少的选择后重试。 |
| C04 | 输出可直接解析 | 单次结果统一 envelope，stdout 无日志、ANSI 或提示；人类模式显式可选，stderr 只承载诊断/进度。Hook 是明确声明的宿主 framing 例外。 |
| C05 | 稳定机器语义 | `schema_version` 与 CLI 版本分开；code、枚举和退出含义有版本边界。未知枚举不降为成功，字段扩展有 schema 与兼容性说明。 |
| C06 | 预览与副作用透明 | Issue/PR 和本地安装写入可 `--dry-run` 查看准确对象、拟变更与前提；不写远端或本地业务状态，必要只读查询在输出中说明。预览不保证之后不会并发改变。 |
| C07 | 写入结果与重试可判断 | 返回 `not_applied`、`applied`、`unknown` 及精确对象或恢复引用；未知先回读，错误提示说明可重试/需核对，不能笼统要求重跑。 |
| C08 | 可组合的对象与分页 | 命令返回稳定 project/request/issue ID 与链接供下一命令使用；列表提供有界分页和未完成标记，不迫使 Agent 抓取人类表格或全量日志。 |
| C09 | 有界与可取消 | 输入/输出/请求数/deadline 有默认上限；Ctrl+C、TERM 与 launcher 退出有契约，不能留下无人管理的 watcher 或 Git 子进程。 |
| C10 | 同一业务实现 | CLI 与 hook 入口调用同一类型化 library；内部不解析自家 CLI JSON，也不递归 spawn 自家 CLI 来实现业务逻辑。 |
| C11 | 授权独立 | schema 中的副作用、`--dry-run` 成功、配置里的偏好或通用 `--yes` 都不授予合并/补关权限；不提供绕过平台或会话约束的开关。 |
| C12 | 安装后通信验证 | 从真正安装的 binary 和 npm launcher 验证 schema、stdin、失败、取消、恢复、两平台对象和 hook framing；开发入口成功不够。 |

随包安装的 Skill 保持简短，列出任务入口、能力发现方式、几个完整组合示例及关键恢复/授权边界；细节按需链接同版本 schema/参考。不要在每个会话注入全量命令说明，也不从 Skill 文案生成一套与实际 CLI 不一致的能力列表。schema 和帮助从同一命令/类型定义生成并验证版本；Skill 是使用说明，不是第二个状态机。
...
| L13 | Agent CLI 通信与文档一致 | C01–C12：离线 schema、JSON/stdin、非交互、dry-run、副作用/恢复语义、分页、取消、hook framing、语言与旧命令迁移诊断 |
...
| L18 | 历史和四层审查 | 全 318 条与旧 F01–F48 有处置；保留项有具体回归/证据；架构越权不会被测试绿灯掩盖 |
