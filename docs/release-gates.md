...
| **I4 idempotent resume** | Re-running any bootstrap/resume command converges on the same delivery — same issues, branch, PR, and record — without duplication. | A resume that forks a second PR or branch for one delivery, or resurrects a merged one. | Holds in the happy path; known corners under repair: [#75](https://github.com/LeXwDeX/SpecGit/issues/75) (post-merge resurrection), [#77](https://github.com/LeXwDeX/SpecGit/issues/77) (same-title adoption) |
