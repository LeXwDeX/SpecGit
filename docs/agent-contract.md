> **Archived SpecGit v1 agent contract.** These rules describe the retired v1 workflow and generated blocks. For current v2 guidance use [Supported coding agents](supported-tools.md), the [native command reference](../runtime/REFERENCE.md), and the [migration guide](migration-v2.md).
...
- For documentation or manual project-guidance changes in this repository, use the [documentation short path](ci-scope.md#documentation-short-path): review the relevant content once and run the existing metadata check. Product TDD, full tests and multi-round code review apply only when the intended change affects product inputs. A green relevant check ends local verification; unrelated findings do not expand the task.
...
- Treat exit `1` and exit `3` differently. Exit `1`: the evidence is complete — fix what the gates named. Exit `3`: evidence is missing — follow `errors[].fix`; use `specgit doctor --json` only for its git, repository, origin, forge-CLI, authentication, and policy probes.
...
| --- | --- | --- |
| `issue` | Starting a delivery | Pass one argument per independently verifiable WHY (quoted titles create; pure numbers reuse). Re-run the same command to resume after a failure between steps; never hand-edit the record instead. A bound request closed without merge is not resumable or replaceable: repair an open draft from the recorded branch with `specgit pr <number>` before starting a new WHY. One issue per WHY — split deliverables that cannot be verified on their own evidence. |
| `finish` | Verify delivery acceptance or inspect completed history | Read-only: report gates, codes, and fixes. On exit 3, follow the named fix and use `doctor` when that diagnostic belongs to its probes; on exit 1, fix the delivery. It never merges or closes issues. |
...
| `init` | Create policy or refresh with `--force` | Use the team's required checks. Require a supported platform before mutation and a proved remote default branch before workflow generation or protection; never guess `main` or select GitHub Enterprise. Automation defaults to no (`--automation no`); only the user's own yes authorizes `--automation yes --merge-target <branch>`. An agent must not answer yes for the user. Ordinary `init --force` preserves saved choices and regenerates the harness; explicit options change configuration. |
| `bind` / `unbind` | Script-level record surgery | Only as machine aliases when a script cannot use `issue`/`pr`. `--issue` takes positive issue numbers for the current forge; `--delivery` only on first bind. |
...
| `doctor` | Diagnosing its prerequisite probes or first-time setup | Its probe order is: git → repo → origin → forge CLI (`gh`/`glab`) → auth → policy. Record, request, CI, and managed-file failures have their own diagnostics and fixes. |

## 5. Working with the PR and checks

- `specgit issue` opens the draft PR/MR with `Closes #n` for every bound issue; keep those references intact when editing the body (keywords: `close(s|d)`, `fix(es|ed)`, `resolve(s|d)`; forms: `#N`, `owner/repo#N`, full issue URL).
...
- Local guidance and entry-point drift warn with `local_assets_stale`; repair them with the exact init/setup commands reported by status, without blocking delivery. Stale, conflicting or partially missing remote acceptance assets still require repair before bootstrap.
