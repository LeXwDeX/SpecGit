...
export function reuseApi(platform: 'github' | 'gitlab', host: string, spawn: SpawnFn = defaultSpawn): ReuseApi {
  return async (endpoint, format = 'json') => {
    ...
    try {
      ...
      let command = platform === 'github' ? process.env.SPECGIT_GH ?? 'gh' : process.env.SPECGIT_GLAB ?? 'glab';
      ...
      if (platform === 'gitlab' && endpoint === 'job') {
        ...
        // Preserve configured executable paths before changing the child's cwd.
        if (command.includes('/') || command.includes(path.sep)) command = path.resolve(command);
