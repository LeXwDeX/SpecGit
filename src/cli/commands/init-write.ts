...
export async function writeHarnessAndPolicy(args: {
  ...
  try {
    desired = await buildHarnessDesiredState(root, { resolveHooksDir, workflowYaml, platform: args.platform, language, completion: args.completion, routingSteps: args.routingSteps });
  } catch (error) {
