...
`}    name: SpecGit Acceptance
    ...
    steps:
      ...
      - name: Isolate trusted engineering source
        if: steps.scope.outputs.build == 'false'
        run: mv .specgit-engineering "$RUNNER_TEMP/specgit-cli"
...
- 对显式声明的整体计划，运行 \`specgit finish --scope <name> --json\`，
...
- \`specgit finish\` is read-only: its verdict comes from real git, PR/MR,
  ...
  the merge and closure of every bound issue and verified derived repair before
  reporting completed.
...
- \`specgit finish\` exit \`0\` means accepted. Report completed only after
  ...
  repair issue and do not require abandoning the original PR/MR. Pending repair
  intents are recovered by the trusted remote runner. Unprovable repairs require
  restored evidence or explicit \`specgit bind --issue <number>\` adoption with
  their closing references retained and the updated delivery reviewed.
