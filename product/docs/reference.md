...
| `automation` | object | Optional, disabled when absent. Strict fields: required `merge`; optional `target_branch`, `close_issues`, and nonempty `repair_labels`. Merge and issue closure can be enabled independently; either enabled operation requires a valid explicit target branch. SpecGit-driven closure requires `close_issues: true`; an omitted or false value does not authorize it. `init --automation yes` writes both operations as enabled unless explicitly overridden. Repair labels must be valid tag slugs and obey project vocabulary. Configure through the user's explicit choice; ordinary `init --force` preserves it. |
...
SpecGit runs only inside a git repository. Root = `git rev-parse --show-toplevel`; record and policy live at that root. No ancestor walking, no global stores. Outside a repository: `not_a_git_repo`, exit 3. Linked-worktree checkouts are first-class (a `.git` file, not directory, is fine). Root discovery removes only git's command terminator, and worktree enumeration uses NUL-delimited records. Spaces and control characters inside real paths are preserved.
...
| G11 checks | every unconditional and applicable check green at PR/MR head | immutable Git diff and provider (`gh`/`glab`) | `checks_missing`, `checks_pending`, `checks_failed` (per check name), `verification_changes_unavailable`, `evidence_truncated` |
...
- `verdict.complete` describes execution coverage: it is true only after every gate ran. A failure short-circuits later gates, which remain `skipped` and make coverage incomplete. This also applies to merged history: proving the merge lineage passes the context gate only; it does not supply missing provider, issue, PR, closing, or check evidence. A fully verified merged delivery still reports `completed` with exit 0.
...
G10 scopes these references to origin. For `acme/web`, `Closes other/web#123` and `Closes https://github.com/other/web/issues/123` cannot close the bound `acme/web#123`. On GitLab the comparison includes every subgroup segment and the declared web authority: `https://git.example.com:8443/group/sub/project/-/issues/123` belongs to that host, port, and project. HTTPS's default port 443 is equivalent to an omitted port. The public `parseClosingRefs(body, dialect)` API retains its numeric extraction behavior; acceptance supplies its optional repository scope.
