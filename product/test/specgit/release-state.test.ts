...
describe('release state and partial publication recovery (#392)', () => {
  it('waits for npm publication visibility before finalizing release metadata (#449)', async () => {
    const { run: publishedRun, calls } = fixture();
    let npmReads = 0;
    const run = (command: string, args: string[]) => {
      if (command === 'npm' && npmReads++ < 2) throw missing();
      return publishedRun(command, args);
    };
    const delays: number[] = [];
  ...
  it('recovers the exact published commit from npm 12 singleton-array metadata (#419)', async () => {
    ...
    await finalizeRelease({ version, run });
    expect(calls).toContainEqual(['git', 'tag', `v${version}`, publishedSha]);
  ...
  ])('refuses $name in successful npm metadata without release writes (#419)', async ({ value }) => {
    const { run, calls } = fixture({ npmOutput: JSON.stringify(value) });
    await expect(finalizeRelease({ version, run })).rejects.toThrow();
  ...
  it('recovers the published source after main advanced, without republishing', async () => {
  ...
  it('recovers a missing release after tag push succeeded', async () => {
  ...
  it('leaves a complete release untouched on repeated retries', async () => {
  ...
  it('refuses to alter a tag pointing at a different source commit', async () => {
    const { run, calls } = fixture({ tag: currentSha });
    await expect(finalizeRelease({ version, run })).rejects.toThrow(/different.*commit/);
    expect(calls.some((call) => call[0] === 'git' && ['tag', 'push'].includes(call[1]))).toBe(false);
  });
  ...
  it.each(['ENOTFOUND', 'E401', 'E500'])('does not retry %s as an unpublished version', async (code) => {
    const error = Object.assign(new Error(code), { stdout: JSON.stringify({ error: { code } }) });
    const { run, calls } = fixture({ npmError: error });
    const delays: number[] = [];
  ...
  it('stops a permanently missing publication at the bounded deadline without release writes (#449)', async () => {
    const { run, calls } = fixture({ published: false });
    const delays: number[] = [];
    await expect(finalizeRelease({
      version,
      ...
      pollMs: 4_000,
      sleep: async (delayMs: number) => { delays.push(delayMs); },
    })).rejects.toThrow(/not published after waiting 10000ms/);
    expect(delays).toEqual([4_000, 4_000, 2_000]);
    expect(calls.filter((call) => call[0] === 'npm')).toHaveLength(4);
    expect(calls.some((call) => call[0] === 'gh' || ['tag', 'push'].includes(call[1]))).toBe(false);
  });
